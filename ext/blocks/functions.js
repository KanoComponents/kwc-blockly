...
import { Blocks } from '../../blocks-module.js';
...
Blocks.function_definition = {
    ...
    getToolbox() {
        let functionDef = this.workspace.functionsRegistry.getFunction(this.id),
            toolbox = functionDef.getParamsXml().map(xml => ({ custom: xml }));
    ...
    _updateCallBlocks() {
        ...
        const funcDef = this.workspace.functionsRegistry.getFunction(this.id);
