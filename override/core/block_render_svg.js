...
/**
 * Minimum height of a larger block.
 * @const
 */
Blockly.BlockSvg.MIN_LARGE_BLOCK_Y = 48;
/**
...
Blockly.BlockSvg.prototype.renderDrawRight_ = function(pathObject, inputRows, iconWidth) {
  ...
  for (var y = 0, row; row = inputRows[y]; y++) {
    ...
    if (this.isCollapsed()) {
      ...
      steps.push('v', (row.height - 2 * Blockly.BlockSvg.CORNER_RADIUS) + Blockly.BlockSvg.STATEMENT_SPACING - 4);
  ...
  if (!inputRows.length) {
    cursorY = this.getMinHeight(Blockly.BlockSvg.MIN_BLOCK_Y);
...
Blockly.BlockSvg.prototype.renderCompute_ = function(iconWidth) {
  ...
  for (var i = 0, input; input = inputList[i]; i++) {
    ...
    input.renderHeight = this.getMinHeight(Blockly.BlockSvg.MIN_BLOCK_Y);
    // If large field Object used, resize block
    if (input.fieldRow[1] && input.fieldRow[1].size_.height > input.renderHeight) {
      input.renderHeight = this.getMinHeight(Blockly.BlockSvg.MIN_LARGE_BLOCK_Y);
    }
 ...
 * @param {!Array.<!Blockly.Field>} input Input with list of Field Rows
...
Blockly.Block.prototype.getMinHeight = function (height) {
    return height + (this.getPaddingY() * 2);
};
